...
void TeleControlInit(){
  gpio.GPIO_PTx=PTD;
  gpio.GPIO_Pins=GPIO_Pin11|GPIO_Pin12|GPIO_Pin13|GPIO_Pin14;
  gpio.GPIO_Dir=DIR_INPUT;
...
void ButtonISR(){
      ...
      if(!PTE2_I){
        SwitchDelay();
      if(!PTE2_I){
      if(!PTB8_I)D_Balance+=0.4;
      if(!PTB16_I)D_Turn+=0.2;
        //D_Turn+=0.2;
        }
      }
    }
